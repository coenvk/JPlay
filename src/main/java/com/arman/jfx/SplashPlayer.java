...
public class SplashPlayer extends Application {
    ...
    public void start(Stage primaryStage) {
        new SplashScreen(primaryStage,
                new Image(getClass().getResource(SPLASH_IMAGE).toExternalForm()), getTask(), this::showApp);
    }
    ...
    public Task<Boolean> getTask() {
        Task<Boolean> task = new Task<Boolean>() {
            ...
            protected Boolean call() {
                ...
                updateProgress(0, dir.list().length);
                players = new ArrayList<>();
                for (int i = 0; i < dir.list().length; i++) {
                    String file = dir.list()[i];
                    updateProgress(i + 1, dir.list().length);
    ...
    private MediaPlayer createPlayer(String mediaSrc) {
        ...
        mp.setOnError(() -> System.out.println("Media error occurred: " + mp.getError()));
        ...
        google.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                try {
                    ...
                    if (!artist.equals(Song.UNKNOWN)) {
                        if (!uri.equals(GOOGLE)) {
                            uri += "+";
                        }
                        uri += artist;
                    }
                    if (!album.equals(Song.UNKNOWN)) {
                        if (!uri.equals(GOOGLE)) {
                            uri += "+";
                        }
                        uri += album;
                    }
                    if (title.equals(Song.UNKNOWN) && artist.equals(Song.UNKNOWN) && album.equals(Song.UNKNOWN)) {
    ...
    private StackPane createEqualizerPane() {
        ...
        for (int i = 0; i < bands.length; i++) {
            ...
            String tooltip = String.valueOf((int) (((bands[i].getValue() + 12) / 24) * 100));
            bands[i].setTooltip(new Tooltip(tooltip));
        ...
        previousMode.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent event) {
    ...
    private void equalize() {
        ...
        for (int i = 0; i < bands.length; i++) {
            int x = i;
            bands[x].valueProperty().addListener(new ChangeListener<Number>() {
                @Override
                public void changed(ObservableValue<? extends Number>
                                            observable, Number oldValue, Number newValue) {
                    double nextGain = newValue.doubleValue();
                    String tooltip = String.valueOf((int) (((nextGain + 12) / 24) * 100));
                    bands[x].setTooltip(new Tooltip(tooltip));
